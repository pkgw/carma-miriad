...
$Log$
Revision 1.1.1.1  1990/09/28 21:50:07  teuben
UIUC 15-feb-2001 w/out wip and some manuals

 * Revision 1.1  90/06/05  21:02:17  mfolk
 * Initial revision
 * 
*/
 ...
 *---------------------------------------------------------------------------*/
    ...
    FCALLKEYW int32
duif2i_(hdim,vdim,max,min,hscale,vscale,data,palette,
                                   outfile,ct_method,hres,vres,compress,lenfn)

    int  *hdim, *vdim; /* horizontal and vertical dimensions of input data */
    float *max, *min,  /* maximum and minimum values in the data */
            hscale[]   /* horizontal and vertical scales */,
            vscale[],
            data[];    /* input data */
    _fcd palette;      /* palette to be stored with the image */
         ...
         *lenfn;       /* length of outfile string */
{
    ...
    fn = DFIf2cstring(outfile, *lenfn);
    ret = DFUfptoimage((int32) *hdim, (int32) *vdim, 
...
#define HORIZ   1   /* horizonal dimension */
#define VERT    0   /* vertical dimension  */
...
DFUfptoimage(hdim,vdim,max,min,hscale,vscale,data,palette,
...
float32 max, min,        /* maximum and minimum values in the data */
        *hscale,*vscale, /* horizontal and vertical scales */
        *data;           /* input data */
unsigned char *palette;  /* palette to be stored with the image */
 ...
 * Returns: 0 on success, -1 on failure with DFerror set
 * Users:    DFUfptoimage
 * Invokes: from libdf.a: DFR8setpalette, DFopen, DFclose, DFR8addimage
...
struct Output *out;
{
    ...
    if (in->is_pal) {
        ret = DFR8setpalette(out->palette);  /* write out as HDF palette */
        if (ret < 0) {
            printf(" Error: %d, preparing to write out palette.\n");
            exit(1);
        }
    }

    dfile = DFopen(out->outfile, DFACC_WRITE, 0);
    DFclose(dfile);

    dimsizes[VERT] = in->vdim;      /* get array dimensions */ 
    dimsizes[HORIZ] = in->hdim;

    /*
    *  allocate buffers for output and scales 
    */
    if (!in->is_hscale)
       in->hscale = (float32 *) malloc((unsigned) (1+in->hdim)*sizeof(float32));
    if (!in->is_vscale)
       in->vscale = (float32 *) malloc((unsigned) (1+in->vdim)*sizeof(float32));
    out->hres = (out->hres <= in->hdim) ? in->hdim : out->hres ;
    out->vres = (out->vres <= in->vdim) ? in->vdim : out->vres ;
    out->image = (char *) malloc((unsigned) (out->hres * out->vres));
    ...
    */
    ...
    ret = DFR8addimage(out->outfile,out->image,
                             out->hres,out->vres,out->compress);
    if (ret < 0)
        printf("Error adding image to file %s\nHDF error number: %d\n\n",
                out->outfile, DFerror);
    /*
    *  free allocated space
    */
    if (!in->is_hscale) free((char *)in->hscale);
    if (!in->is_vscale) free((char *)in->vscale);
    free((char *)out->image);
...
struct Output *out;
{
    int i,j;
    printf("\n");
      for(i=0;i<out->vres && i<20; i++) {
          printf("\n");
          for (j=0; j< out->hres; j++)
             if (j<19)
                printf("%4d",(unsigned char) out->image[i*out->hres + j]);
      }
     printf("\n");
...
struct Output  *out;
{
    ...
    p = (unsigned char *) out->image; /* space for interpolated image */
    ...
    dxs = (float32 *)malloc((unsigned) (sizeof(float)*out->hres) );
												/* temp space for dx's */
    dys = (float32 *)malloc((unsigned) (sizeof(float)*out->vres) );
												/* temp space for dy's */
    xinc = (char *)malloc((unsigned)out->hres);
    yoffs = (int32 *)malloc((unsigned)(out->vres+1)*sizeof(int));
    yoffs[0] = 0;
    ...
    if (yrange > 0) {
        for (i=0; i<out->vres; i++) {        /* fill in dy's */
            pt = dely*(float)i + *in->vscale;/* scaled position in new image */
...
*/
    ...
    for (i= 0; i < out->vres; i++,yv++) {
        ...
        for (j=0; j < out->hres; j++,xv++) {    /* for each target point */
...
struct Output  *out;
{
    ...
    register unsigned char *image, *prevrow;
    unsigned char *pixvals;
    int i, theval, *hoffsets, *voffsets, prevoffset;

    data = in->data;                          /* space for data */
    image = (unsigned char *) out->image;  /* space for image */

    range = in->max - in->min;
    if (range < 0) range = -range;            /* max must be > min */
    
    hoffsets = (int *)malloc((unsigned)(out->hres+1)*sizeof(int));
    voffsets = (int *)malloc((unsigned)(out->vres+1)*sizeof(int));
    pixvals = (unsigned char *)malloc((unsigned)in->hdim+1);
    ...
    for (i=0; i<out->vres; i++) {   /* for each row, store pixel vals */
        ...
        if (voffsets[i] > prevoffset) {  /* if new data row, compute pix vals */
            ...
            for (j=0; j<in->hdim; j++) {  /* compute vals for each data point*/
                ...
                pixvals[j] = (unsigned char) theval;
            }
...
compute_offsets(scale,dim,offsets, res)
float32 *scale;
...
int *offsets;
{
    ...
    midpt = (float32 *) malloc((unsigned int) (sizeof(float32)*dim) );
...
{
    int i,j;
    unsigned char raster_val;
    register unsigned char *image, *row_buf;
    float32 *in_row_ptr, *in_buf;
    float32 ratio, delh, delv, hblockend, vblockend;

    ratio = 237.9 / (in->max - in->min);
    image = (unsigned char *) out->image;
    in_buf = in->data;

    delh = ( (float) out->hres) / in->hdim;  /* horiz block size */
    delv = ( (float) out->vres) / in->vdim;  /* vert block size  */
...
*/
    ...
    for(i=0; i<out->vres; i++, vblockend += delv) { 
        ...
        for(j=0; j<out->hres; j++, hblockend += delh) { 
            ...
            raster_val = (unsigned char)
                             (1.5 + ratio * (float) (*in_row_ptr++-in->min));
            *image++ = raster_val;

            for (; j<hblockend-1; j++) /* store vals for this blk of this row*/
                *image++ = raster_val;
        }
