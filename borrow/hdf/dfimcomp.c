...
$Log$
Revision 1.1.1.1  1990/09/28 21:49:56  teuben
UIUC 15-feb-2001 w/out wip and some manuals

 * Revision 3.4  90/07/02  10:12:07  clow
...
unsigned char *new_pal;          /* pointer to new palette           */
...
static unsigned char *image;	/* contains the compressed image            */
...
char *out, *out_pal;
{
  ...
  if (color_pt) DFIfreespace((char *) color_pt);
  color_pt = (struct rgb *)DFIgetspace((unsigned)((xdim*ydim)/8) *
				       sizeof(struct rgb));
...
int blocks;
{
  ...
  for (i=0; i<(2*blocks); i++)
  {
    ...
    if (trans[k] == -1)
    {
      r = (unsigned char) color_pt[i].c[RED]<<3;
      g = (unsigned char) color_pt[i].c[GREEN]<<3;
      b = (unsigned char) color_pt[i].c[BLUE]<<3;
...
struct rgb *color_pt;
{
  ...
  /* alloc memory */
  if (hist) DFIfreespace((char *) hist);
  if (distinct_pt) DFIfreespace((char*) distinct_pt);
  hist = (int *)DFIgetspace((unsigned)distinct * sizeof(int));
  distinct_pt = (struct rgb *)DFIgetspace((unsigned)distinct *
					  sizeof(struct rgb));
  ...
  /* set up first box */
  first = (struct box *)DFIgetspace(sizeof(struct box));
  for (i=RED; i<=BLUE; i++)
  {
  ...
  first->pts = (int *)DFIgetspace((unsigned)distinct * sizeof(int));
  for (i=0; i<distinct; i++)
    first->pts[i] = i;
  first->nmbr_pts = 2*blocks;
  first->nmbr_distinct = distinct;

  dummy = (struct box *)DFIgetspace(sizeof(struct box));
  frontier = dummy;
  dummy->right = first;
  first->left = dummy;
  first->right = NULL;
  ...
  DFIfreespace((char *)first);
  DFIfreespace((char *)dummy);
} /* end of init */
...
struct box *ptr;
{
  ...
  /* create 2 child */
  l_child = (struct box *)DFIgetspace(sizeof(struct box));
  r_child = (struct box *)DFIgetspace(sizeof(struct box));
...
int dim;
{
  ...
  rank = (int *)DFIgetspace((unsigned)ptr->nmbr_distinct * sizeof(int));
  for (i=0; i<ptr->nmbr_distinct; i++)
    rank[i] = ptr->pts[i];
...
struct box *ptr, *child;
{
  ...
  temp = (int *)DFIgetspace((unsigned)ptr->nmbr_distinct * sizeof(int));
  ...
  child->pts = (int *)DFIgetspace((unsigned)distinct * sizeof(int));
  for (i=0; i<distinct; i++)
    child->pts[i] = temp[i];

  DFIfreespace((char *)temp);
