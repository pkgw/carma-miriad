...
PRIVATE FILE *INIT_OUTPUT NOARGS{

	FILE *STDOUT;

	if ((STDOUT = fdopen(fileno(stdout), "w")) == NULL){
		fprintf(stderr, "Error: get_http could not massage STDOUT\n");
		exit(1);
	}
...
PRIVATE void SET_OUTPUT ARGS1(HTStream *, me){

	FILE *output;

	if (! strcmp(me->format->name, "application/x-tar") ){
		if ((output = popen("tar -xf -", "w")) == (FILE *)NULL){
			fprintf(stderr, "Error: get_http could not open pipe to tar\n");
			exit(1);
		}
	}
	else{
	...
	me->request->output_stream = (HTStream *)HTFWriter_new(output, NO);
