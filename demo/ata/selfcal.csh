...
rm -r $config.$dec.uv
uvgen ant=$config.ant baseunit=-3.33564 radec=23:23:25.803,$dec harange=$harange source=$MIRCAT/point.source telescop=hatcreek systemp=40 jyperk=150 freq=$freq corr=$nchan,1,1,.1 out=$config.$dec.uv pnoise=$pnoise gnoise=$gnoise
echo UVGEN: `date` >> timing

echo selfcal
selfcal vis=$config.$dec.uv interval=.01 options=amp,noscale
