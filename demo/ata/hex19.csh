#!/bin/csh -vf
echo "   ---  ATA Mosaicing (Cas A model)   ---   "
echo "   mchw. 01nov02 version"
...
#  01nov02 mchw. Re-import from ALMA to ATA.
...
calc '3.05/(pi*1000)*12'
# 1.1650141E-02 hours
set harange = 0,0.19,0.01
...
#  lambda/2*antdiam (arcsec) = 3463 arcsec at 1.420 GHz
calc "300/$freq/2/6.1e3*2e5"
hex rings=3 cell=3600 | awk '{if(NR>1)printf("%.2f,%.2f\n", $1,$2)}' > hex19

echo "Using hex7 mosaic with 3600'' spacing" >> casa.results

echo "Generate uv-data. Tsys=40K, Jy/K=150,  bandwidth=100 MHz " >> timing
rm -r $config.uv
uvgen ant=$config.ant baseunit=-3.33564 radec=23:23:25.803,$dec harange=$harange source=$MIRCAT/no.source systemp=40 jyperk=150 freq=$freq corr=$nchan,1,0,100 out=$config.uv telescop=hatcreek center=@hex19
...
implot in=$config.$dec.cas.$cell.mp device=/xs units=s
...
rm -r $config.$dec.cas.$cell.mem $config.$dec.cas.$cell.cm
mosmem  map=$config.$dec.cas.$cell.mp beam=$config.$dec.cas.$cell.bm out=$config.$dec.cas.$cell.mem niters=200 flux=732.063 region=$region rmsfac=1
goto restor
