...
# check inputs
  if($#argv<3) then
    echo " Usage:  hex7.csh array declination cell "
    echo "   e.g.  hex7.csh config1 30 0.04"
    echo " Inputs :"
    echo "   array"
...
set imsize  = 129
set imsize  = 257
set region  = 'arcsec,box(15,-15,-15,15)'
...
echo "Using hex7 mosaic with 12'' spacing" >> casa_default.results
...
echo "Joint deconvolution of interferometer and single dish data ; niters=200 rmsfac=200,1" >> casa_default.results
...
echo " MOSMEM Interferometer only with niters=200 flux=732.063 rmsfac=200." >> casa_default.results
rm -r $config.$dec.cas.$cell.mem $config.$dec.cas.$cell.cm
...
echo "MOSMEM with default single dish image; niters=200 rmsfac=200"  >> casa_default.results
rm -r $config.$dec.cas.$cell.mem_default $config.$dec.cas.$cell.cm
mosmem map=$config.$dec.cas.$cell.mp default=single.$dec.cas.$cell.map beam=$config.$dec.cas.$cell.bm out=$config.$dec.cas.$cell.mem_default region=$region niters=200 rmsfac=200


restor:
restor map=$config.$dec.cas.$cell.mp beam=$config.$dec.cas.$cell.bm out=$config.$dec.cas.$cell.cm model=$config.$dec.cas.$cell.mem_default
...
#  mv $config.$dec.cas.$cell.conv.gif ~/public_html
  ...
  rm -r $config.$dec.cas.$cell.resid_default
  imdiff in1=$config.$dec.cas.$cell.cm in2=$config.$dec.cas.$cell.regrid resid=$config.$dec.cas.$cell.resid_default options=nox,noy,noex
  implot device=/xs units=s in=$config.$dec.cas.$cell.resid_default region=$region
  histo in=$config.$dec.cas.$cell.resid_default region=$region
...
set SRMS = `histo in=$config.$dec.cas.$cell.resid_default region=$region | grep Rms | awk '{printf("%.3f   ", $4)}'`
set SMAX = `histo in=$config.$dec.cas.$cell.resid_default region=$region | grep Maximum | awk '{printf("%.3f   ", $3)}'`
set SMIN = `histo in=$config.$dec.cas.$cell.resid_default region=$region | grep Minimum | awk '{printf("%.3f   ", $3)}'`
...
echo  " "
echo  "$config  $dec  $harange  $RMS   $b1 $b2    $cell  $Model_Flux $Model_Peak  $Flux $Peak   $SRMS  $SMAX  $SMIN  $Fidelity" >> casa_default.results
mv timing $config.$dec.$harange.$nchan.$imsize
cat $config.$dec.$harange.$nchan.$imsize
cat casa_default.results
#enscript -r casa_default.results
