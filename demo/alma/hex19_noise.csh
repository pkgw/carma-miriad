...
#  15dec02 mchw. Add RMS noise to single dish image.
...
# check inputs
  if($#argv<4) then
    echo " Usage:  hex19_noise.csh array declination cell rms "
    echo "   e.g.  hex19_noise.csh config1 30 0.04 3.0"
    echo " Inputs :"
    echo "   array"
    ...
    echo "   rms"
    echo "          rms (Jy) to add to single dish image. No default."
    echo " "
    exit 1
  endif
...
echo "Add noise to single dish image. RMS=$rms"  >> casa.results
rm -r single.$dec.cas.$cell.mp
imgen in=single.$dec.cas.$cell.map  out=single.$dec.cas.$cell.mp object=noise spar=$rms

puthd in=single.$dec.cas.$cell.mp/rms value=$rms
rm -r junk
imdiff in1=single.$dec.cas.$cell.map  in2=single.$dec.cas.$cell.mp resid=junk
histo in=junk
...
rm -r $config.$dec.cas.$cell.mem $config.$dec.cas.$cell.cm
 mosmem  map=$config.$dec.cas.$cell.mp,single.$dec.cas.$cell.mp beam=$config.$dec.cas.$cell.bm,single.$dec.cas.$cell.beam out=$config.$dec.cas.$cell.mem niters=200 region=$region rmsfac=200,1
goto restor
...
rm -r $config.$dec.cas.$cell.mem $config.$dec.cas.$cell.cm
mosmem map=$config.$dec.cas.$cell.mp default=single.$dec.cas.$cell.mp beam=$config.$dec.cas.$cell.bm out=$config.$dec.cas.$cell.mem region=$region niters=200 rmsfac=200
