...
set nvis = `invert options=mfs vis=$config.$dec.uv map=$config.$dec.mp beam=$config.$dec.bm imsize=$imsize fwhm=.36,.31 select=$select | grep Visibilities | awk '{print $3}'`
