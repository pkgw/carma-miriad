...
echo UNIFORM weighting -- after uvdata is already made.
...
set nvis = `invert options=mfs vis=$config.$dec.uv map=$config.$dec.mp beam=$config.$dec.bm imsize=$imsize select=$select | grep Visibilities | awk '{print $3}'`
