...
# ACA - using 120 Jy/K
rm -r $ACA.uv
uvgen ant=$ACA.ant baseunit=-3.33564 radec=23:23:25.803,$dec lat=-23.02 harange=$harange source=$MIRCAT/no.source systemp=40 jyperk=120 freq=$freq corr=$nchan,1,0,8000 out=$ACA.uv telescop=alma
...
invert vis=ALMA.$dec.$model.$cell.uv,ACA.$dec.$model.$cell.uv map=$config.$dec.$model.$cell.mp beam=$config.$dec.$model.$cell.bm imsize=$imsize sup=0 options=mosaic,double select=$select
