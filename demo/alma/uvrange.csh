...
rm -r $config.$dec.uv
uvgen ant=$config.ant baseunit=-3.33564 radec=23:23:25.803,$dec lat=-23.02 harange=$harange source=$MIRCAT/point.source telescop=alma systemp=$systemp jyperk=40 freq=$freq corr=$nchan,1,1,8000 out=$config.$dec.uv
...
echo " " >> timing
echo " Config  DEC  Nchan HA[hrs]  Rms[\muJy]  Beam[arcsec] Tb_rms[\muK] Sidelobe[%]:Rms,Max,Min Nvis[%] uvrange[m]" >> timing
echo  "$config  $dec  $nchan  $harange  $RMS   $BMAJ $BMIN    $TBRMS   $SRMS  $SMAX  $SMIN  $nvis $uvrange" >> timing
echo " "
echo  "$config  $dec  $nchan  $harange   $RMS   $BMAJ x $BMIN   $TBRMS   $SRMS   $SMAX   $SMIN   $Nvis $uvrange" >> uvrange.results
mv timing $config.$dec.$harange.$nchan.$imsize.$nvis
cat $config.$dec.$harange.$nchan.$imsize.$nvis
tail uvrange.results
