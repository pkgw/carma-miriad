...
# mchw 30jun04 - heteroDZ.csh get uvmin from SZA, uvmax from CARMA antennas. 
...
uvgen ant=$config.ant baseunit=-3.33564 radec=23:23:25.803,$dec lat=37.02 harange=$harange source=$MIRCAT/no.source systemp=80,290,0.26 jyperk=167 freq=$freq corr=$nchan,1,0,4000 out=cross.uv telescop=ovro ellim=$ellim
...
uvgen ant=$config.ant baseunit=-3.33564 radec=23:23:25.803,$dec lat=37.02 harange=$harange source=$MIRCAT/no.source systemp=80,290,0.26 jyperk=383 freq=$freq corr=$nchan,1,0,4000 out=sza.uv telescop=sza ellim=$ellim
...
uvplt vis=cross.$dec.uv,carma.$dec.uv,sza.$dec.uv axis=uc,vc options=nobase,equal select=$select device=$config.$dec.cps/cps
