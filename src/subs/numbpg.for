C* NUMBPG -- convert a number into a plottable character string
C& MJS
C: STRINGS
C+
      SUBROUTINE NUMBPG (MM, PP, FORM, STRING, NC)
      INTEGER MM, PP, FORM
      CHARACTER*(*) STRING
      INTEGER NC
C
...
C  History:
C
C     mjs  29mar92 Taken as-is from PGPLOT's "pgnumb" to avoid the
C                  need to load all of PGPLOT when only that one
C                  routine is needed (relevent to shared library
C                  implementations).
C     mjs  14jun93 Incorporate TJP's mod to original "pgnumb".
C-----------------------------------------------------------------------
      CHARACTER*1 BSLASH
