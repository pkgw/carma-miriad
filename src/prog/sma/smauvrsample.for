c************************************************************************
        program smauvrsample
        implicit none
c
c= smauvrsample - Resample Sma UVdata 
c& jhz 22-sep-04
c: data transfer/re-sample
c+
c       SMAUVRSAMPLE is a MIRIAD task, which resamples an original uv data-set 
c       from an higher spectral resolution to a lower spectral resolution
c       or from hybrid spectral resolutions to a uniform spectral resolution.
c       This is a useful tool in handling hybrid spectral resolution data.
c
...
c       The default is 16. Note that this number must be power of 2 and  
c       equal or less than the smallest channel number in the 24 
c       spectral windows.
c
c@ sideband
...
c--
c  History:
c    jhz 22-Sep-04 did the first version 
c------------------------------------------------------------------------
        integer maxfiles
        parameter(maxfiles=128)
        character version*(*)
        parameter(version='SmaUVRsample: version 1.1 01-Dec-04')
c
...
c   
        ...
        call hiswrite(tno,'SMAUVRSAMPLE: Miriad '//version)
        call hisinput(tno,'SMAUVRSAMPLE')
        ifile = 0
        iostat = 0
     ...
     *          iostat)
        ...
        if(iostat.ne.0)then
          ...
          call hiswrite(tno,'SMAUVRSAMPLE: '//line)
          call hiswrite(tno,
     *      'SMAUVRSAMPLE: Prematurely finishing because of errors')
        endif
        call hisclose(tno)
        call uvclose(tno)
        if(.not.dsb) then
