      program tsysmed
c
c= tsysmed - Redo the Tsys correction, using a median smoothed Tsys curve
c	     
c& nebk
c: calibration
c+
c     TSYSMED redoes the Tsys correction by constructing a median
...
c---------------------------------------------------------------------------
      ...
      character version*(*)
      parameter(version='version 22-Aug-02', frqmax=2,
     +          polmax = 2, fldmax = 200, antmax=16, timemax=3000,
...
c     +                   tsys(1,ifield,ia1,ipol1,ifreq), 
...
c
c        if (abs(freqs(i)-freq).le.0.25) ifreq = i
c      end do
      ifreq = 1
c
c      if (ifreq.eq.0) return
    
c tao
c ignore name and freq
      keep = .true.
      ifield = 1
      return

c     gmx: More dead code
c
c Field
c
c      ifield = 0
c      il1 = len1(source)
c      il2 = len(sroot)
c      ip = index(source,sroot(1:il2))
c      if (ip.ne.0) then
c        ip = il2 + 1
c        call atoif(source(ip:il1), ifield, ok)
c      end if
c      if (ifield.lt.ifield1 .or. ifield.gt.ifield2) ifield = 1
c
c      if (ifield*ifreq.ne.0) keep = .true.
c
      end
c
c
...
c-----------------------------------------------------------------------
c      call output (' ')
c      call output ('Begin new time series')
c
...
c            
          ...
          call pgsci(1);
c          call output ('Another smoothing length ?')
c          call output ('Click left for yes, right for no')
c          call pgcurs (xx, yy, ch)
c          if (ch.eq.'x' .or. ch.eq.'X') then
          done = .true.
c         endif
        end do
c
...
c  
c        call output (' ')
c        call output ('Click left to edit, right to finish')
c        call pgcurs (xx, yy, ch)
        ch = 'x'
