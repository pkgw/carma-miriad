...
c+
	subroutine moduvDatInp(In,startch,endch,flags)
c
	implicit none
	character In*(*),flags*(*)
        real startch,endch
...
c
c	if(doref)then
c	  call keyrline(ref,rstart,rwidth)
c	  doref = ref.ne.' '
c	endif
c
...
c
        call bug('w',"STOKES")
	if(dostokes)call moduvPolInp(maxPol,nPol,Pols)
        call bug('w',"STOKES DONE")
c
...
c
	...
	SelPol1 = .false.
c	if(dosels)then
c	  call SelInput('select',sels,maxsels)
c	  SelPol = SelProbe(sels,'polarization?',0.d0)
c	  if((dostokes.or.doleak).and.SelPol)then
c	    i = PolMin
c	    n = 0
c	    dowhile(i.le.PolMax.and.n.lt.2)
c	      if(SelProbe(sels,'polarization',dble(i))) n = n + 1
c	      i = i + 1
c	    enddo
c	    SelPol1 = n.eq.1
c	  endif
...
c============================================================================
c* uvDatPy -- Get uv data parameters.
c& dnf
c: uv-i/o,uv-data,uv-selection
c+
	subroutine moduvDatPy(vis1,vis2,ltype,nc,ls,lw,lst,lf,refer,
     *                     rs,rw,flags)
c
	implicit none
	character vis1*(*),vis2*(*),flags*(*),refer*(*),ltype*(*)
        real rs,rw,ls,lw,lst,lf
        integer nc
c
c  This gets inputs and store them away in common. It remembers
c  which uvset calls we need to make. In particular, it gets the following
c
c  Input:
c    vis1/2     The visibility file names
c    ltype      The line type
c    nc         number of channels
c    ls         starting channel
c    lw         channel bin width
c    lst        channel step size
c    lf         line flags
c    refer      reference line type
c    rs         reference start channel
c    rw         reference line width
c    flags	This is a character string determining the processing steps
c		that need to be performed. It consists of:
c		 'r'	Get reference linetype specification (keyword 'ref').
c		 's'	Get Stokes/polarisations (keyword 'stokes').
...
c
	...
	docal	 = index(flags,'c').gt.0
	dopass   = index(flags,'f').gt.0
	dostokes = .false. !index(flags,'s').gt.0
	auto	 = index(flags,'a').gt.0
	cross	 = index(flags,'x').gt.0
	dosingle = index(flags,'b').gt.0
	dosels   = .false. !index(flags,'d').gt.0
	doleak	 = index(flags,'e').gt.0
	dow	 = index(flags,'3').gt.0
	if(dow)then
	  npream = 5
...
c************************************************************************
	subroutine moduvPolInp(maxPol,nPol,Pols)
c
	implicit none
...
c: uv-i/o,uv-data,uv-selection
c+
	logical function moduvDatOpn(tIn)
c
	implicit none
	integer tIn
c
...
c: uv-i/o,uv-data,uv-selection
c+
	subroutine moduvDatRd(preamble,data,flags,n,nread)
c
	implicit none
	integer n,nread
...
c
	if(WillPol)then
	  call moduvPolGet(preamble,data,flags,n,nread)
	else
...
c************************************************************************
	subroutine moduvPolGet(preamble,data,flags,n,nread)
c
	double precision preamble(*)
	integer n,nread
	complex data(n)
...
c
	...
	moduvPolIni = Snread.eq.0
	if(moduvPolIni)return
c
...
c: uv-i/o,uv-data,uv-selection
c+
	subroutine moduvDatWRd(data,flags,n,nread)
c
	implicit none
	integer n,nread
	complex data(n)
...
c: uv-i/o,uv-data,uv-selection
c+
	subroutine moduvDatGti(object,ival)
c
	implicit none
	character object*(*)
	integer ival(*)
...
c: uv-i/o,uv-data,uv-selection
c+
	subroutine moduvDatGtr(object,rval)
c
	implicit none
	character object*(*)
	real rval
...
c: uv-i/o,uv-data,uv-selection
c+
	subroutine moduvDatGta(object,aval)
c
	implicit none
...
c: uv-i/o,uv-data,uv-selection
c+
	subroutine moduvDatSet(object,value)
c
	implicit none
	character object*(*)
	integer value
...
c: uv-i/o,uv-data,uv-selection
c+
	logical function moduvDatPrb(object,value)
c
	implicit none
	character object*(*)
...
c
	logical SelProbe
c
	moduvDatPrb = SelProbe(sels,object,value)
	end
c************************************************************************
	subroutine moduvLkIni()
c
	implicit none
c
...
c************************************************************************
	subroutine moduvLkCorr(baseline,maxPol,ncoeff,type,coeffs,
     *	  Leaks,nLeaks)
c
	implicit none
