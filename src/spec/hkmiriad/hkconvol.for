...
c@ edgetc
c       truncate edges of image using Fermi-Dirac type function
c       e.g. edgetc=a,b
c        image = image * 1/(1+exp(-(x-a)/b)) * 1/(1+exp(-(nx-a-x)/b))
c
c--
c  History:
...
c
	...
	call keyr('edgetc',edgea,0.)
	call keyr('edgetc',edgew,20.)
	call keyfin
c
...
c
c  Truncate edges with Fermi-Dirac type function
c
	  if (edgea .gt. 0.0) then
	     do i = 0,nx*ny-1
		iy = i / nx
		ix = i-iy*nx
		fx = 1./(1.+exp(-(ix-edgea)/edgew))
     *                 /(1.+exp(-(nx-edgea-ix)/edgew))
		fy = 1./(1.+exp(-(iy-edgea)/edgew))
     *                 /(1.+exp(-(ny-edgea-iy)/edgew))
		memR(pDat+i) = memR(pDat+i) *fx *fy
	     enddo
	  endif
c
c  Do the real work.
